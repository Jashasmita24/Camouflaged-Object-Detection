"""
Training Script for CODNet - Camouflaged Object Detection.
...
from utils.losses import CODLoss
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train CODNet")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) % config.SAVE_EVERY == 0 or is_best:
            ...
            save_path = os.path.join(
                ...
                f"codnet_epoch{epoch+1}.pth"
            )
            torch.save(ckpt, save_path)
            print(f"Saved checkpoint: {save_path}")

            if is_best:
                best_path = os.path.join(config.CHECKPOINT_DIR, "codnet_best.pth")
